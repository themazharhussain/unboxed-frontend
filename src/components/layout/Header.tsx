...
export default function Header() {
  ...
  // Prevent body scroll when menu is open — pad to avoid layout shift
  useEffect(() => {
    if (open) {
  ...
  return (
    <header className="relative z-50 w-full lg:fixed lg:top-0 lg:left-0 lg:right-0 lg:backdrop-blur-sm">
      <div className="mx-auto flex h-18 w-full max-w-330 items-center justify-between px-4 lg:px-8">
        ...
        <nav className="hidden items-center gap-8 text-base font-medium text-primary lg:flex">
          {links.map(({ href, label }) => (
            <Link key={href} href={href} className=" text-primary hover:opacity-70 transition-opacity">
              {label}
            </Link>
          ))}
        </nav>
